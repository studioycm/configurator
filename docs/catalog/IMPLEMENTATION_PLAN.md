...
- [x] Inspect intended deployment source read-only and finalize retained table/column/file/job mapping. Actual `ari_configurator` inspected during the user-authorized development promotion; 32 tables, retained rows, ten media files and old jobs verified separately from the local rehearsal.
...
| T13 | Rehearsal and authorized development deployment complete; client release content pending | Primary `master` deployed to `ari.data4.work` with `ari_configurator_rebuild`; remote legacy backup restored into `ari_configurator_legacy_copy`, all 32 table hashes and ten media hashes matched, source unchanged. 501 imports/reimports and seven filters verified; D060 unassigned. No synthetic definitions deployed. See [release evidence](RELEASE_REHEARSAL.md) for workflow and remaining client content/sign-in check. |
...
Completed: application commit `62e7bb4`, GitHub primary `master`, original project folder on `master` and local `configurator_catalog_dev`; Forge deployment `78450920` finished at 03:18:57 UTC. Actual browser verified remote Home, D060 501→85 filtering, Product facts/unassigned state, theme persistence and admin login form. The source remote database has zero changed table fingerprints. D060 is unassigned locally and remotely; Preview & Test remains placeholder-only. Full suite after relocation again passed **245 / 1370**. The filter seed was initially rejected by automatic approval review due to an incorrect assumption that it assigned QA; reading the exact seeder proved it creates filters only, and the subsequent approved invocation verified `configurator_id = null`, seven filters and zero remote Configurators.

### User amendment — full-width filter choices and legacy library (2026-09-24)

Filter choices now stack as full-width buttons inside each responsive filter column. Browser verification confirmed equal column/button widths and Working Pressure filtering from 501 to 85 Products. The outer compact grid is retained.

The user authorized reusing the actual preserved legacy library. A private read-only export contains 13 Attributes and 35 Options, SHA-256 `817404483c876dc9e949d4510884770e915b7a17beaf0296422a47af489916e3`. Five codes are each reused by two legacy Options. Their ten rows remain pending the user's decision on code uniqueness; global uniqueness has not been changed and no replacement code has been invented. The original database and archive remain untouched.

`catalog:import-legacy-library` defaults to read-only review, requires an admitted `--actor`, and requires the reviewed `--source-hash` with `--apply`. Explicit `--hold-duplicate-codes` leaves every member of each duplicate-code set pending while importing independent rows. It uses the canonical save actions in one transaction, rejects conflicting existing definitions, preserves exact codes and identities on repeat, and creates separate Values rather than merging labels. Private reports map legacy IDs to canonical IDs. Configurator-local defaults, order, hints and rules are outside this library import; no Configurator or Group assignment is created.

Local apply created 13 Attributes, 25 Options and 25 Values; repeat review proposes zero new records. D060 remains unassigned. Targeted SQLite and isolated MySQL import/canonical tests: **14 passed / 98 assertions** on each. Full SQLite suite: **253 passed / 1428 assertions**. Pint, Vite build and diff checks passed. Private source exports and reports are excluded from Git. Deployment and remote import evidence will be appended after verification.
