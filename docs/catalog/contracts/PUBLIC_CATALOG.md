...
Order cards by `product_code`, then Product ID. Select only IDs/route identity, `product_code`, `product_name` and explicit pressure/connection fact aliases needed for cards. **Do not select or serialize complete Product models/properties, `parts` or `extra_data` into Livewire public state.** Metadata/aggregate extraction happens on the server. Discovery does not load configurator definitions, media or POC configuration relations. A source `product_pic` path is not yet a validated public image URL; use an honest absence/placeholder rather than automatic remote media lookup.
...
ProductConfigurator receives a locked Product identity, resolves current assignment/definition server-side on each interaction and calls the core annex's `ConfiguratorDefinitionLoader`, compiler and engine. The public adapter and admin Preview & Test consume the same result. Territory/Application are each single-select with `All` unrestricted; session persistence is deferred. Illegal/hidden/disabled options are rejected/refreshed, without clearing upstream choices to make them legal. Discovery's newest-choice reconciler is never reused here. Imported parts and future assets/specification mapping are not implied by discovery or placeholders; **Custom** remains the exact placeholder tab label.
