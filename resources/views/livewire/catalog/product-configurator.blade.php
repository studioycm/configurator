<section class="flex flex-col gap-3 lg:flex-row" aria-label="{{ __('Product configuration') }}" wire:loading.attr="aria-busy" x-data="{ tab: 'configurator' }">
    <aside class="w-full shrink-0 space-y-3 lg:w-52" aria-label="{{ __('Product context') }}">
        <div class="rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-zinc-700 dark:bg-zinc-900">
            <p class="text-sm font-semibold">{{ auth()->user()?->name }}</p>
            <p class="mt-1 text-sm text-slate-500">{{ $product->group->name }}</p>
        </div>
        @if ($result->definition)
            @foreach (['territory', 'application'] as $dimension)
                <label class="block rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-zinc-700 dark:bg-zinc-900" wire:key="context-{{ $dimension }}">
                    <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __(ucfirst($dimension)) }}</span>
                    <select wire:change="changeContext('{{ $dimension }}', $event.target.value)" class="w-full rounded-lg border border-slate-300 bg-transparent px-2 py-1 text-sm dark:border-zinc-600">
    ...
    <div class="min-w-0 flex-1 space-y-3">
        <div class="space-y-2 rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-zinc-700 dark:bg-zinc-900">
            <h2 class="text-lg font-semibold">{{ $product->product_name ?: $product->group->name }}</h2>
        ...
        <div id="panel-configurator-{{ $this->getId() }}" role="tabpanel" aria-labelledby="tab-configurator-{{ $this->getId() }}" x-show="tab === 'configurator'" class="grid items-start gap-3 xl:grid-cols-[minmax(10rem,1fr)_minmax(0,3fr)]">
            <div class="rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-zinc-700 dark:bg-zinc-900">
                <x-catalog.product-facts :product="$product" :buckets="['properties' => 'Product facts']" :property-keys="$product->group->filters->isNotEmpty() ? $product->group->filters->pluck('property_key')->all() : array_slice(array_keys($product->properties ?? []), 0, 7)" />
            </div>
        ...
        <div id="panel-details-{{ $this->getId() }}" role="tabpanel" aria-labelledby="tab-details-{{ $this->getId() }}" x-cloak x-show="tab === 'details'" class="rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-zinc-700 dark:bg-zinc-900">
            <div class="grid items-start gap-4 md:grid-cols-2">
                ...
                <x-catalog.product-facts :product="$product" :buckets="['properties' => 'Specifications and dimensions']" />
            </div>
        </div>
