<div data-catalog-state="{{ json_encode($result?->state->toArray()) }}" data-catalog-repair="{{ $result?->repaired ? '1' : '0' }}" data-catalog-url="{{ route('catalog.groups.show', ['group' => $group, 'd' => $result?->state->toArray()]) }}">
    ...
    @else
        <section aria-label="Product filters" class="mt-3">
            <div class="mb-3 flex flex-wrap items-center gap-4">
                ...
                <button type="button" wire:click="resetAll" class="min-h-8 rounded-full border border-slate-300 bg-white px-3 text-sm hover:bg-slate-100 dark:bg-zinc-900">{{ __('Reset all') }}</button>
            </div>
            @if ($result->subGroups !== [])
                <fieldset class="mb-3 min-w-0" aria-labelledby="catalog-subgroup-title-{{ $group->id }}">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach ($result->subGroups as $preset)
                            ...
                            <button type="button" wire:key="preset-{{ $group->id }}-{{ $preset['id'] }}"
                                ...
                                @class(['inline-flex min-h-8 max-w-full items-center gap-2 rounded-full border px-3 py-1 text-left text-base leading-5 transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-600',
                                    'border-blue-600 bg-blue-600 text-white' => $selected,
            ...
            @if ($result->fields !== [])<p id="catalog-choice-help" class="mt-2 text-xs text-zinc-600 dark:text-zinc-400">{{ __('Muted choices have no matches with your other choices. A newer choice may clear incompatible earlier choices. Clear filters keeps your subgroup; Reset all removes it too.') }}</p>@endif
        </section>
        <section class="mt-4" aria-label="Products" wire:loading.attr="aria-busy">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-4">
                @if ($result->showProducts && $result->settings['allow_page_size_change'])
                    <label class="flex items-center gap-2 text-sm">{{ __('Products per page') }}
                        <select wire:change="changePageSize(+$event.target.value)" class="min-h-8 rounded-lg border border-zinc-300 bg-white px-3 dark:border-zinc-600 dark:bg-zinc-900">
            ...
            <span wire:loading.delay class="mb-2 text-sm">{{ __('Updating products…') }}</span>
