...
<article class="flex h-full flex-col rounded-xl border border-zinc-300 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
    ...
    @if ($values->isNotEmpty())
        <p class="my-5 text-sm leading-6 wrap-break-word text-zinc-600 dark:text-zinc-400" aria-label="{{ __('Product properties') }}">{{ $values->implode(', ') }}</p>
    @endif
    <a class="mt-auto inline-flex min-h-11 items-center pt-2 font-medium text-teal-700 underline underline-offset-4 dark:text-teal-300" href="{{ route('catalog.products.show', $product->id) }}">{{ __('View product') }}<span class="sr-only">: {{ $product->product_code }}</span></a>
</article>
